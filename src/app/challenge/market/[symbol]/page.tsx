...
export default async function ChallengeStockDetailPage({
  params,
}: {
  ...
  const [quote, membershipResult] = await Promise.all([
  ...
  const cashBalance = Number(membershipResult.data?.league_cash_balance ?? 0);
