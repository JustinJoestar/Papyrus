...
class AnimationController {
  ...
  private drawTrail(t1: number) {
    for (let i = 0; i < this.trailLength; i++) {
      const f = this.map(i, 0, this.trailLength, 1, 0)
      const sw = (2.0 + 3.0 * Math.sin(Math.PI * t1)) * f
...
class Star {
  ...
  constructor(cameraZ: number, cameraTravelDistance: number) {
    ...
    const rawZ = zMin + Math.random() * (zMax - zMin)
    this.z = lerp(rawZ, cameraTravelDistance / 2, 0.3 * this.spiralLocation)
  ...
  render(p: number, controller: AnimationController) {
    ...
    if (q > 0) {
      ...
      if (displacementProgress < 0.3) {
        const t = displacementProgress / 0.3
        ...
        const targetDistance = this.distance * this.expansionRate * this.finalScale * 1.5
